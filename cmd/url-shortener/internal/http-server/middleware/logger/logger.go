...
func New(log *slog.Logger) func(next http.Handler) http.Handler { // input: func(next http.Handler) output  func(next http.Handler) http.Handler
	return func(next http.Handler) http.Handler { // принимаем следующий middleware next http.Handler , отдаем  func(next http.Handler) http.Handler
		log = log.With(
